...
func (l *Logger) Debug(msg string, ctx Context) {
	l.log(msg, ctx, slog.LevelDebug)
}

func (l *Logger) Info(msg string, ctx Context) {
	l.log(msg, ctx, slog.LevelInfo)
}

func (l *Logger) Warn(msg string, ctx Context) {
	l.log(msg, ctx, slog.LevelWarn)
}

func (l *Logger) Error(msg string, ctx Context) {
	l.log(msg, ctx, slog.LevelError)
}

func (l *Logger) log(msg string, ctx Context, lvl Level) {
	// The logging API is context-free by design; slog's default JSON handler
	// ignores the context.
	l.logger.LogAttrs(
		context.Background(),
		lvl,
